function keep(S::T, idxs) where {T<:FlowSample}
    params = merge(S.params, Dict("\$TOT" => string(length(idxs))))
    data = S.data[:, idxs]
    FlowSample{T.parameters[1], Vector{Int}}(data, params)
end

function apply!(S::FlowSample, f, channels...)
    for channel in channels
        S.data[channel, :] = f.(S[channel, :])
    end
    S
end
